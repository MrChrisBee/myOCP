...
public class MessageFormatDemo {
	public static void main(String[] args) {
		...
		for (Locale local : locales) {
			ResourceBundle rb = ResourceBundle.getBundle("wbs.localization.ResourceBundle", local);
			...
			MessageFormat.format(rb.getString("muster"), new Date(), 42);
			String output = MessageFormat.format(rb.getString("muster"),new Date(), 42);
			System.out.println(output);
			System.out.println("--------");
		}
		// default locale
		ResourceBundle rb = ResourceBundle.getBundle("wbs/i18n/Messages");
		System.out.println(rb.getString("danke"));
		// ändern des default locale
		Locale.setDefault(Locale.FRENCH);
		rb = ResourceBundle.getBundle("wbs/i18n/Messages");
		System.out.println(rb.getString("danke"));
	}
}